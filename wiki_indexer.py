...
def processInfo(text):
    ...
    st="}}"
    for line in data:
        if re.match(r'\{\{ज्ञानसन्दूक', line) or re.match(r'\{\{Infobox', line) or re.match(r'\{\{infobox', line):
            if re.match(r'\{\{ज्ञानसन्दूक', line):
                info.append(re.sub(r'\{\{ज्ञानसन्दूक(.*)', r'\1', line))
                flag = 0
    ...
    # data = remove_stopwords(data)
    # data = stem(data)
    # if not data[2].isspace() and not data[3].isspace():
    #     print(data[2], data[3])
    return info
...
class WikiXmlHandler(xml.sax.handler.ContentHandler):
    ...
    def endElement(self, tag):
        ...
        if tag == 'page':
            ...
            if 'फ़िल्म' in title and len(info) > 0: 
                print(title.rstrip())
                # for i in info:
                #     print(i)
                # print("Infobox -- ", info)
